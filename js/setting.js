...
export class Setting {
  ...
  api() {
    ...
    let Api = `http://api.weatherapi.com/v1/forecast.json?key=a109b9a47aec4cbf923164830232701&q=${seaValue}&days=3`;
    return Api;
  }
  async searchData() {
    let response = await this.fetchApi(this.api()).finally(
      console.log("missed name")
    );
    $("#countryName").html(response.location.name);
    $("#condition").html(response.current.condition.text);
    $("#iconMain").html(
      `<img class="p-1" src="${response.current.condition.icon}" alt="">`
    );
  ...
  async nextDays() {
    ...
    $("#iconNext1").html(
      `<img  src="${response.forecast.forecastday[0].day.condition.icon}" alt="">`
    );
